...
/**
 * FIRHTTPMetric object can be used to make the SDK record information about a
...
/**
 * Use `initWithURL:HTTPMethod:` for Objective-C and `init(url:httpMethod:)` for
 * Swift.
 */
...
/**
 * Marks the end time of the response and queues the network request metric on
 * the device for transmission. Check the logs if the metric is valid.
 */
- (void)stop;
