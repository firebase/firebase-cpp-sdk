...
/**
 * FIRTrace objects contain information about a "Trace", which is a sequence of
 * steps. Traces can be used to measure the time taken for a sequence of steps.
 * Traces also include "Counters". Counters are used to track information which
 * is cumulative in nature (e.g., Bytes downloaded). Counters are scoped to an
 * FIRTrace object.
 */
...
/**
 * Increments the counter for the provided counter name by 1. If it is a new
 * counter name, the counter value will be initialized to 1. Does nothing if the
 * trace has not been started or has already been stopped.
 *
 * Note: This API has been deprecated. Please use -incrementMetric:byInt:
 * instead.
 *
 ...
 * If it is a new counter name, the counter value will be initialized to the
 * value. Does nothing if the trace has not been started or has already been
 * stopped.
 *
 * Note: This API has been deprecated. Please use -incrementMetric:byInt:
 * instead.
...
/**
 * Atomically increments the metric for the provided metric name with the
 * provided value. If it is a new metric name, the metric value will be
 * initialized to the value. Does nothing if the trace has not been started or
...
/**
 * Gets the value of the metric for the provided metric name. If the metric
 * doesn't exist, a 0 is returned.
 *
...
/**
 * Sets the value of the metric for the provided metric name to the provided
 * value. Does nothing if the trace has not been started or has already been
 * stopped.
 *
...
- (void)setIntValue:(int64_t)value
          forMetric:(nonnull NSString*)metricName
