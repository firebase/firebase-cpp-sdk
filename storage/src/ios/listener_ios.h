...
class ListenerInternal {
 ...
 private:
  ...
  // listener_handle_.
  NSRecursiveLock* _Nonnull listener_handle_lock() const {
    return listener_handle_lock_->get();
  }
  ...
  FIRStorageHandle _Nullable pause_observer_handle() const {
    return pause_observer_handle_->get();
  }
  ...
  FIRCPPStorageListenerHandle* _Nullable listener_handle() const {
    return listener_handle_->get();
  }
#endif  // __OBJC__
