...
#pragma clang attribute push(                                             \
        __attribute__((external_source_symbol(                            \
                language = "Swift", defined_in = "FirebaseStorage",       \
...
/// Firebase Storage is a service that supports uploading and downloading binary
/// objects, such as images, videos, and other files to Google Cloud Storage.
/// Instances of <code>Storage</code> are not thread-safe. If you call
/// <code>Storage.storage()</code>, the instance will initialize with the
/// default <code>FirebaseApp</code>, <code>FirebaseApp.app()</code>, and the
/// storage location will come from the provided
/// <code>GoogleService-Info.plist</code>.
/// If you provide a custom instance of <code>FirebaseApp</code>,
/// the storage location will be specified via the
/// <code>FirebaseOptions.storageBucket</code> property.
SWIFT_CLASS_NAMED("Storage")
@interface FIRStorage : NSObject
/// The default <code>Storage</code> instance.
///
/// returns:
/// An instance of <code>Storage</code>, configured with the default
/// <code>FirebaseApp</code>.
+ (FIRStorage *_Nonnull)storage SWIFT_WARN_UNUSED_RESULT;
/// A method used to create <code>Storage</code> instances initialized with a
/// custom storage bucket URL. Any <code>StorageReferences</code> generated from
/// this instance of <code>Storage</code> will reference files and directories
/// within the specified bucket. \param url The <code>gs://</code> URL to your
...
/// Creates an instance of <code>Storage</code>, configured with a custom
/// <code>FirebaseApp</code>. <code>StorageReference</code>s generated from a
/// resulting instance will reference files in the Firebase project associated
/// with custom <code>FirebaseApp</code>. \param app The custom
/// <code>FirebaseApp</code> used for initialization.
///
///
/// returns:
/// A <code>Storage</code> instance, configured with the custom
/// <code>FirebaseApp</code>.
+ (FIRStorage *_Nonnull)storageForApp:(FIRApp *_Nonnull)app
    SWIFT_WARN_UNUSED_RESULT;
/// Creates an instance of <code>Storage</code>, configured with a custom
/// <code>FirebaseApp</code> and a custom storage bucket URL. \param app The
/// custom <code>FirebaseApp</code> used for initialization.
///
/// \param url The <code>gs://</code> url to your Firebase Storage bucket.
///
///
/// returns:
/// the <code>Storage</code> instance, configured with the custom
/// <code>FirebaseApp</code> and storage bucket URL.
+ (FIRStorage *_Nonnull)storageForApp:(FIRApp *_Nonnull)app
...
/// The maximum time in seconds to retry operations other than upload and
/// download if a failure occurs. Defaults to 2 minutes (120 seconds).
@property(nonatomic) NSTimeInterval maxOperationRetryTime;
/// Specify the maximum upload chunk size. Values less than 256K (262144) will
/// be rounded up to 256K. Values above 256K will be rounded down to the nearest
/// 256K multiple. The default is no maximum.
@property(nonatomic) int64_t uploadChunkSizeBytes;
/// A <code>DispatchQueue</code> that all developer callbacks are fired on.
/// Defaults to the main queue.
@property(nonatomic, strong) dispatch_queue_t _Nonnull callbackQueue;
/// Creates a <code>StorageReference</code> initialized at the root Firebase
/// Storage location.
///
/// returns:
/// An instance of <code>StorageReference</code> referencing the root of the
/// storage bucket.
- (FIRStorageReference *_Nonnull)reference SWIFT_WARN_UNUSED_RESULT;
/// Creates a StorageReference given a <code>gs://</code>, <code>http://</code>,
/// or <code>https://</code> URL pointing to a Firebase Storage location. For
/// example, you can pass in an <code>https://</code> download URL retrieved
/// from <code>StorageReference.downloadURL(completion:)</code> or the
/// <code>gs://</code> URL from <code>StorageReference.description</code>.
/// \param url A gs // or https:// URL to initialize the reference with.
///
///
/// throws:
/// Throws a fatal error if <code>url</code> is not associated with the
/// <code>FirebaseApp</code> used to initialize this Storage instance.
///
/// returns:
...
- (FIRStorageReference *_Nonnull)referenceForURL:(NSString *_Nonnull)url
    SWIFT_WARN_UNUSED_RESULT;
/// Creates a <code>StorageReference</code> initialized at a location specified
/// by the <code>path</code> parameter. \param path A relative path from the
...
/// methods on a new instance of <code>Storage</code>.
...
/// Defines task operations such as pause, resume, cancel, and enqueue for all
/// tasks. All tasks are required to implement enqueue, which begins the task,
/// and may optionally implement pause, resume, and cancel, which operate on the
/// task to pause, resume, and cancel operations.
...
/// and <code>StorageDownloadTask</code>, to provide state transitions, event
/// raising, and common storage for metadata and errors. Callbacks are always
/// fired on the developer-specified callback queue. If no queue is specified,
/// it defaults to the main queue. This class is not thread safe, so only call
...
/// An extended <code>StorageTask</code> providing observable semantics that can
/// be used for responding to changes in task state. Observers produce a
/// <code>StorageHandle</code>, which is used to keep track of and remove
/// specific observers at a later date. This class is not thread safe and can
...
/// Failure. \param status The <code>StorageTaskStatus</code> change to observe.
///
...
/// <code>StorageDownloadTask</code> implements resumable downloads from an
/// object in Firebase Storage. Downloads can be returned on completion with a
/// completion handler, and can be monitored by attaching observers, or
/// controlled by calling <code>pause()</code>, <code>resume()</code>, or
/// <code>cancel()</code>. Downloads can currently be returned as
/// <code>Data</code> in memory, or as a <code>URL</code> to a file on disk.
/// Downloads are performed on a background queue, and callbacks are raised on
/// the developer specified <code>callbackQueue</code> in Storage, or the main
/// queue if left unspecified. Currently all downloads must be initiated and
/// managed on the main queue.
...
/// Adds wrappers for common Firebase Storage errors (including creating errors
/// from GCS errors). For more information on unwrapping GCS errors, see the GCS
/// errors docs: https://cloud.google.com/storage/docs/json_api/v1/status-codes
/// This is never publicly exposed to end developers (as they will simply see an
/// NSError).
typedef SWIFT_ENUM_NAMED(NSInteger, FIRStorageErrorCode, "StorageErrorCode",
                         open){
...
/// <code>StorageReference.list()</code> call.
...
@property(nonatomic, readonly, copy)
    NSArray<FIRStorageReference *> *_Nonnull prefixes;
/// The objects (files) returned by a <code>list()</code> operation.
///
/// returns:
/// A page token if more results are available.
@property(nonatomic, readonly, copy)
    NSArray<FIRStorageReference *> *_Nonnull items;
/// Returns a token that can be used to resume a previous <code>list()</code>
/// operation. <code>nil</code> indicates that there are no more results.
///
/// returns:
/// A page token if more results are available.
...
@property(nonatomic, copy)
    NSDictionary<NSString *, NSString *> *_Nullable customMetadata;
...
- (NSDictionary<NSString *, NSObject *> *_Nonnull)
...
/// <code>StorageReference</code> represents a reference to a Google Cloud
...
/// For example, in <code>gs://bucket/path/to/object.txt</code>, the bucket
/// would be ‘bucket’.
@property(nonatomic, readonly, copy) NSString *_Nonnull bucket;
...
/// In <code>gs://bucket/path/to/object.txt</code>, the name of the object would
/// be <code>object.txt</code>.
...
/// Creates a new <code>StorageReference</code> pointing to the parent of the
/// current reference or <code>nil</code> if this instance references the root
/// location. For example: path = foo/bar/baz   parent = foo/bar path = foo
/// parent = (root) path = (root)        parent = nil
///
/// returns:
/// A new <code>StorageReference</code> pointing to the parent of the current
/// reference.
- (FIRStorageReference *_Nullable)parent SWIFT_WARN_UNUSED_RESULT;
/// Creates a new <code>StorageReference</code> pointing to a child object of
...
///
/// returns:
/// A new <code>StorageReference</code> pointing to a child location of the
/// current reference.
- (FIRStorageReference *_Nonnull)child:(NSString *_Nonnull)path
    SWIFT_WARN_UNUSED_RESULT;
/// Asynchronously uploads data to the currently specified
/// <code>StorageReference</code>, without additional metadata. This is not
/// recommended for large files, and one should instead upload a file from disk.
/// \param uploadData The data to upload.
///
/// \param metadata <code>StorageMetadata</code> containing additional
/// information (MIME type, etc.) about the object being uploaded.
///
///
/// returns:
/// An instance of <code>StorageUploadTask</code>, which can be used to monitor
...
/// Asynchronously uploads data to the currently specified
/// <code>StorageReference</code>. This is not recommended for large files, and
/// one should instead upload a file from disk. \param uploadData The data to
/// upload.
///
///
/// returns:
...
/// information (MIME type, etc.) about the object being uploaded.
///
/// \param completion A closure that either returns the object metadata on
...
/// <code>StorageReference</code>. <code>putData</code> should be used instead
/// of <code>putFile</code> in Extensions. \param fileURL A URL representing the
...
- (FIRStorageUploadTask *_Nonnull)putFile:(NSURL *_Nonnull)fileURL
                                 metadata:
                                     (FIRStorageMetadata *_Nullable)metadata;
/// Asynchronously uploads a file to the currently specified
/// <code>StorageReference</code>, without additional metadata.
/// <code>putData</code> should be used instead of <code>putFile</code> in
/// Extensions.
...
/// information (MIME type, etc.) about the object being uploaded.
///
/// \param completion A completion block that either returns the object metadata
...
- (FIRStorageUploadTask *_Nonnull)
    ...
    completion:(void (^_Nullable)(FIRStorageMetadata *_Nullable,
                                  NSError *_Nullable))completion;
/// Asynchronously downloads the object at the <code>StorageReference</code> to
/// a <code>Data</code> instance in memory. A <code>Data</code> buffer of the
/// provided max size will be allocated, so ensure that the device has enough
/// free memory to complete the download. For downloading large files,
/// <code>write(toFile:)</code> may be a better option. \param maxSize The
/// maximum size in bytes to download. If the download exceeds this size, the
/// task will be cancelled and an error will be returned.
///
/// \param completion A completion block that either returns the object data on
/// success, or an error on failure.
///
///
/// returns:
/// An <code>StorageDownloadTask</code> that can be used to monitor or manage
/// the download.
...
/// developer in the Firebase Console. \param completion A completion block that
/// either returns the URL on success, or an error on failure.
///
...
/// system filepath. <ul>
///   <li>
///     Returns An <code>StorageDownloadTask</code> that can be used to monitor
...
/// Asynchronously downloads the object at the current path to a specified
/// system filepath. \param fileURL A file system URL representing the path the
/// object should be downloaded to.
///
/// \param completion A closure that fires when the file download completes,
/// passed either a URL pointing to the file path of the downloaded file on
/// success, or an error on failure.
///
///
/// returns:
/// A <code>StorageDownloadTask</code> that can be used to monitor or manage the
/// download.
- (FIRStorageDownloadTask *_Nonnull)
...
/// <code>StorageReference</code>. This is a helper method for calling
/// <code>list()</code> repeatedly until there are no more results. Consistency
/// of the result is not guaranteed if objects are inserted or removed while
/// this operation is executing. All results are buffered in memory.
/// <code>listAll(completion:)</code> is only available for projects using
/// Firebase Rules Version 2. \param completion A completion handler that will
/// be invoked with all items and prefixes under the current
/// <code>StorageReference</code>.
///
- (void)listAllWithCompletion:(void (^_Nonnull)(FIRStorageListResult *_Nullable,
                                                NSError *_Nullable))completion;
/// List up to <code>maxResults</code> items (files) and prefixes (folders)
...
/// <code>list(maxResults:completion:)</code> is only available for projects
...
/// <code>maxResults</code> items and prefixes under the current
/// <code>StorageReference</code>.
///
- (void)listWithMaxResults:(int64_t)maxResults
                completion:(void (^_Nonnull)(FIRStorageListResult *_Nullable,
                                             NSError *_Nullable))completion;
/// Resumes a previous call to <code>list(maxResults:completion:)</code>,
/// starting after a pagination token. Returns the next set of items (files) and
/// prefixes (folders) under this <code>StorageReference</code>. “/” is treated
/// as a path delimiter. Storage does not support unsupported object paths that
/// end with “/” or contain two consecutive “/“s. All invalid objects in GCS
/// will be filtered. <code>list(maxResults:pageToken:completion:)</code>is only
/// available for projects using Firebase Rules Version 2. \param maxResults The
/// maximum number of results to return in a single page. Must be greater than 0
/// and at most 1000.
///
/// \param pageToken A page token from a previous call to list.
///
/// \param completion A completion handler that will be invoked with the next
...
/// \param metadata A <code>StorageMetadata</code> object with the metadata to
/// update.
///
/// \param completion A completion block which returns the
/// <code>StorageMetadata</code> on success, or an error on failure.
///
...
/// An error raised during task execution, or <code>nil</code> if no error
/// occurred.
...
typedef SWIFT_ENUM_NAMED(NSInteger, FIRStorageTaskStatus, "StorageTaskStatus",
                         open){
...
/// <code>StorageUploadTask</code> implements resumable uploads to a file in
/// Firebase Storage. Uploads can be returned on completion with a completion
/// callback, and can be monitored by attaching observers, or controlled by
/// calling <code>pause()</code>, <code>resume()</code>, or
/// <code>cancel()</code>. Uploads can be initialized from <code>Data</code> in
/// memory, or a URL to a file on disk. Uploads are performed on a background
/// queue, and callbacks are raised on the developer specified
/// <code>callbackQueue</code> in Storage, or the main queue if unspecified.
