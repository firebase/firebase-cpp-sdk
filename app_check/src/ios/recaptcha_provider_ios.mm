...
#include "app_check/src/ios/recaptcha_provider_ios.h"
...
class RecaptchaProvider : public AppCheckProvider {
 public:
  RecaptchaProvider(FIRRecaptchaProvider* provider);
  virtual ~RecaptchaProvider();
...
void RecaptchaProvider::GetToken(
...
void RecaptchaProvider::GetLimitedUseToken(
    std::function<void(AppCheckToken, int, const std::string&)> completion_callback) {
...
RecaptchaProviderFactoryInternal::RecaptchaProviderFactoryInternal() : created_providers_() {
  ios_provider_factory_ = std::make_unique<FIRRecaptchaProviderFactoryPointer>(
      [[FIRRecaptchaProviderFactory alloc] init]);
}

RecaptchaProviderFactoryInternal::~RecaptchaProviderFactoryInternal() {
...
AppCheckProvider* RecaptchaProviderFactoryInternal::CreateProvider(App* app) {
  ...
  FIRRecaptchaProvider* ios_provider =
      [ios_provider_factory() createProviderWithApp:app->GetPlatformApp()];
  AppCheckProvider* cpp_provider = new internal::RecaptchaProvider(ios_provider);
...
RecaptchaProviderFactory::RecaptchaProviderFactory()
    : internal_(new internal::RecaptchaProviderFactoryInternal()) {}
