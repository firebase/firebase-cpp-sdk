...
class IntegerListNode {
 ...
 private:
  ...
  // Disallow copying.
  IntegerListNode(const IntegerListNode&);
  IntegerListNode& operator=(const IntegerListNode&);
};
...
TEST_F(intrusive_list_test, splice_empty) {
  ...
  firebase::intrusive_list<IntegerListNode> other_list(&IntegerListNode::node);
...
TEST_F(intrusive_list_test, move_constructor) {
  ...
  firebase::intrusive_list<IntegerListNode> other(std::move(list_));
...
TEST_F(intrusive_list_test, move_assignment) {
  ...
  firebase::intrusive_list<IntegerListNode> other(&IntegerListNode::node);
...
TEST_F(intrusive_list_test, erase_iterator) {
  ...
  for (int i = 0; i < 10; ++i) {
    e[i] = new IntegerListNode(i);
    list_.push_back(*e[i]);
  }

  // Test that erase(iterator) works.
  for (int i = 0; i < 10; ++i) {
    EXPECT_EQ(list_.size(), 10 - i);

    using iterator = firebase::intrusive_list<IntegerListNode>::iterator;
    iterator it = std::find(list_.begin(), list_.end(), IntegerListNode(i));
    iterator next_it = list_.erase(it);
    if (i == 9) {
      EXPECT_EQ(next_it, list_.end());
    } else {
      EXPECT_NE(next_it->value(), i);
    }

    EXPECT_EQ(list_.size(), 10 - i - 1);
    delete e[i];
  }
}
...
TEST_F(intrusive_list_test, erase_range) {
  ...
  for (int i = 0; i < 10; i += 2) {
    ...
    EXPECT_EQ(result, range_end);
    if (i + 2 == 10) {
      EXPECT_EQ(result, list_.end());
    } else {
      EXPECT_EQ(result->value(), i + 2);
    }
    EXPECT_EQ(list_.size(), 10 - i - 2);
    delete e[i];
    delete e[i + 1];
  }
