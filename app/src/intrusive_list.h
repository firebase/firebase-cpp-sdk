...
// This code was copied (rather than reused) to avoid a dependency on fplutil.
// But any changes here should likely be reflected there also.
...
#if !defined FIREBASE_NOEXCEPT
#if defined(__clang__)
#if __has_feature(cxx_noexcept)
#define FIREBASE_NOEXCEPT noexcept
#endif
...
#endif
#endif
#ifndef FIREBASE_NOEXCEPT
#define FIREBASE_NOEXCEPT
#endif
#endif
...
//   int value;
//   firebase::intrusive_list_node node;
// };
//
...
// struct ExampleStruct {
//   int value;
//   firebase::intrusive_list_node node_a;
//   firebase::intrusive_list_node node_b;
// };
class intrusive_list_node {
 public:
...
class intrusive_list {
 ...
 public:
  ...
  static void insert_before(reference value,
                            reference other,
  ...
  static void insert_after(reference value,
                           reference other,
                           intrusive_list_node T::*node_member) {
  ...
  iterator erase(iterator pos) {
    node_type* next = pos.value_->next_;
    pos.value_->remove();
    return iterator(next, node_offset_);
  }

  iterator erase(iterator first, iterator last) {
    if (first == last) return last;
    node_type* before_first = first.value_->previous_;
    last.value_->previous_ = before_first;
    before_first->next_ = last.value_;

    iterator iter = first;
    while (iter != last) {
      iterator current = iter++;
      current.value_->clear();
    }
    return iter;
  }
 ...
 private:
  ...
  // For example, if you have an intrusive list 'my_list', you can iterate over
  // it like so:
  //
  ...
  class intrusive_list_iterator {
   public:
    ...
    typedef
        typename emulate_std::conditional<is_const,
