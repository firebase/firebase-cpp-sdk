...
set - e
    ...
    if[["$(uname)" != "Darwin"]];
then echo
    "Error: Frameworks and universal libraries can only be built on a Mac "
    "host." exit 2 fi

        packagepath = $1 includepath = $2 binutils = $3 os = $4

    if[[-z "${os}"]];
then os = darwin fi

    if[[-z "${packagepath}" || -z "${includepath}" || -z "${binutils}"]];
then echo
    "Usage: $0 <packaged iOS/Mac SDK path> <include file path> <binutils path> "
    "[darwin|ios]" exit 2 fi

    if[[!-d "${packagepath}/libs/${os}"]];
then echo "Can't find packaged iOS/Mac SDK in ${packagedpath}/libs/${os}" fi

    if[[!-r "${includepath}/firebase/app.h"]];
then echo "Can't find include files in ${includepath}" exit 2 fi
...
for a in $(cd "${packagepath}"/libs/"${os}"; find . -depth 1 -type d | sed 's|^\./||');
do
  if
    [[$a != "universal"]];
then architectures += ($a)fi done
...
libname=$(basename ${lib})
xcrun lipo "${packagepath}/libs/${os}"/*/
    "${libname}" -
    create -
    output "${packagepath}/libs/${os}/universal/${libname}" done

#Done, remove old library backups.
        rm -
    f "${packagepath}/libs/${os}"/*/*.bak
