...
void FirebaseMessagingTest::SetUpTestSuite() {
  ...
  initializer.Initialize(
    ...
    *listener = new firebase::messaging::PollableListener();
    ...
    std::string value;
    if (!GetPersistentString(kObtainedPermissionKey, &value) || value.empty()) {
...
void FirebaseMessagingTest::SendTestMessage(
    ...
    std::string request;
    std::map<std::string, std::string> headers;
    EXPECT_TRUE(CreateTestMessage(send_to, notification_title,
                                  notification_body, message_fields, &request,
                                  &headers));
...
TEST_F(FirebaseMessagingTest, TestSubscribeAndUnsubscribe) {
    ...
    EXPECT_TRUE(WaitForCompletion(
        firebase::messaging::Subscribe("SubscribeTest"), "Subscribe"));
    EXPECT_TRUE(WaitForCompletion(
...
TEST_F(FirebaseMessagingTest, TestNotification) {
    ...
    const char kNotificationBody[] =
        "Test notification, open to resume testing.";
    std::string value;
    if (!GetPersistentString(kTestingNotificationKey, &value) ||
        value.empty()) {
...
TEST_F(FirebaseMessagingTest, TestSendMessageToTopic) {
    ...
    SendTestMessage(("/topics/" + topic).c_str(), "Topic Title 2",
                    "Topic Body 2",
                    {{"message", "Hello, world!"}, {"unique_id", unique_id}});
...
TEST_F(FirebaseMessagingTest, TestChangingListener) {
    ...
    SendTestMessage(shared_token_->c_str(), kNotificationTitle,
                    kNotificationBody,
                    {{"message", "Hello, world!"}, {"unique_id", unique_id}});
    LogDebug("Waiting for message.");
    firebase::messaging::Message message;
