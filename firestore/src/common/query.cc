...
Query Query::WhereEqualTo(const std::string& field, const FieldValue& value) {
  return WhereEqualTo(FieldPath::FromDotSeparatedString(field), value);
}

Query Query::WhereEqualTo(const FieldPath& field, const FieldValue& value) {
  if (!internal_) return {};
...
Query Query::WhereLessThan(const FieldPath& field, const FieldValue& value) {
  if (!internal_) return {};
...
Query Query::WhereGreaterThan(const FieldPath& field, const FieldValue& value) {
  if (!internal_) return {};
...
Query Query::OrderBy(const FieldPath& field, Direction direction) {
  if (!internal_) return {};
  return internal_->OrderBy(field, direction);
}

Query Query::Limit(int32_t limit) {
  if (!internal_) return {};
...
Query Query::StartAt(const std::vector<FieldValue>& values) {
  if (!internal_) return {};
  return internal_->StartAt(values);
}

Query Query::StartAfter(const DocumentSnapshot& snapshot) {
  if (!internal_) return {};
  return internal_->StartAfter(snapshot);
}

Query Query::StartAfter(const std::vector<FieldValue>& values) {
  if (!internal_) return {};
  return internal_->StartAfter(values);
}

Query Query::EndBefore(const DocumentSnapshot& snapshot) {
  if (!internal_) return {};
  return internal_->EndBefore(snapshot);
}

Query Query::EndBefore(const std::vector<FieldValue>& values) {
  if (!internal_) return {};
  return internal_->EndBefore(values);
}

Query Query::EndAt(const DocumentSnapshot& snapshot) {
  if (!internal_) return {};
  return internal_->EndAt(snapshot);
}

Query Query::EndAt(const std::vector<FieldValue>& values) {
  if (!internal_) return {};
