...
#include "firebase_firestore-Swift.h"
...
FirestoreInternal::FirestoreInternal(
  ...
  // Use Swift-C++ Interop via ObjC interface for now to ensure stability
  swift_bridge_ = (__bridge_retained void*) [[FirestoreBridge alloc] initWithApp:app->GetPlatformApp()];
...
Settings FirestoreInternal::settings() const {
  FirestoreBridge* bridge = (__bridge FirestoreBridge*)swift_bridge_;
  FirestoreSettings* swiftSettings = bridge.settings;
  
  Settings result;
  result.set_host(std::string(swiftSettings.host.UTF8String));
  result.set_ssl_enabled(swiftSettings.isSSLEnabled);
  result.set_persistence_enabled(swiftSettings.isPersistenceEnabled);
  result.set_cache_size_bytes(swiftSettings.cacheSizeBytes);
  return result;
}

void FirestoreInternal::set_settings(Settings from) {
  FirestoreBridge* bridge = (__bridge FirestoreBridge*)swift_bridge_;
  FirestoreSettings* swiftSettings = [[FirestoreSettings alloc] init];
  
  swiftSettings.host = [NSString stringWithUTF8String:from.host().c_str()];
  swiftSettings.sslEnabled = from.is_ssl_enabled();
  swiftSettings.persistenceEnabled = from.is_persistence_enabled();
  swiftSettings.cacheSizeBytes = from.cache_size_bytes();
  
  bridge.settings = swiftSettings;

  // Also keep C++ core in sync for now
...
Future<void> FirestoreInternal::DisableNetwork() {
  ...
  FirestoreBridge* bridge = (__bridge FirestoreBridge*)swift_bridge_;
  [bridge disableNetworkWithCompletion:^(NSError* error) {
    if (error) {
      promise.SetError(Status(Error::kErrorUnknown, error.localizedDescription.UTF8String));
    } else {
      promise.SetValue();
    }
  }];
...
Future<void> FirestoreInternal::EnableNetwork() {
  ...
  FirestoreBridge* bridge = (__bridge FirestoreBridge*)swift_bridge_;
  [bridge enableNetworkWithCompletion:^(NSError* error) {
    if (error) {
      promise.SetError(Status(Error::kErrorUnknown, error.localizedDescription.UTF8String));
    } else {
      promise.SetValue();
...
Future<void> FirestoreInternal::Terminate() {
  ...
  // Note: FirestoreBridge does not expose terminate yet, keeping Core call or adding to Bridge?
  // User asked for specific async methods. Terminate was not explicitly in my previous edit list but is async.
  // I will keep Core call for Terminate for now as I didn't add it to Bridge.
...
Future<void> FirestoreInternal::WaitForPendingWrites() {
  ...
  FirestoreBridge* bridge = (__bridge FirestoreBridge*)swift_bridge_;
  [bridge waitForPendingWritesWithCompletion:^(NSError* error) {
    if (error) {
      promise.SetError(Status(Error::kErrorUnknown, error.localizedDescription.UTF8String));
    } else {
      promise.SetValue();
...
Future<void> FirestoreInternal::ClearPersistence() {
  ...
  FirestoreBridge* bridge = (__bridge FirestoreBridge*)swift_bridge_;
  [bridge clearPersistenceWithCompletion:^(NSError* error) {
    if (error) {
      promise.SetError(Status(Error::kErrorUnknown, error.localizedDescription.UTF8String));
    } else {
      promise.SetValue();
