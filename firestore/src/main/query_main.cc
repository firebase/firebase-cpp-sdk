...
Query QueryInternal::Where(const Filter& filter) const {
  core::Filter core_filter = GetInternal(&filter)->filter_core(query_, user_data_converter_);
...
Query QueryInternal::Where(const FieldPath& field_path,
  ...
  api::Query decorated = query_.AddNewFilter(query_.ParseFieldFilter(path, op, std::move(parsed), describer));
  return MakePublic(std::move(decorated));
}
