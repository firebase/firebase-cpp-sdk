...
FilterInternal::FilterInternal(FilterInternal::FilterType filter_type) : filter_type_(filter_type) {}
...
Filter FilterInternal::UnaryFilter(const FieldPath& field_path, UnaryOperator op, const std::vector<FieldValue>& values) {
  return MakePublic(UnaryFilterInternal(field_path, op, values));
}

template <typename ... Filters>
Filter FilterInternal::CompositeFilter(core::CompositeFilter::Operator op, const Filter& filter, const Filters&... filters) {
  ...
  if (lhs.filter_type_ == rhs.filter_type_) {
    switch (lhs.filter_type_) {
      case FilterInternal::Composite:
        return *static_cast<const CompositeFilterInternal*>(&lhs) == *static_cast<const CompositeFilterInternal*>(&rhs);
      case FilterInternal::Unary:
        return *static_cast<const UnaryFilterInternal*>(&lhs) == *static_cast<const UnaryFilterInternal*>(&rhs);
    }
  }
  return false;
}
