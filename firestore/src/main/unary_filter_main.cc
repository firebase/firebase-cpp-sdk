...
UnaryFilterInternal::UnaryFilterInternal(FieldPath field_path, core::FieldFilter::Operator op, FieldValue value)
    : FilterInternal(FilterType::Unary), path_(std::move(field_path)), op_(op), value_(std::move(value)){}

UnaryFilterInternal::UnaryFilterInternal(FieldPath field_path, core::FieldFilter::Operator op, const std::vector<FieldValue>& values)
    : FilterInternal(FilterType::Unary), allow_arrays_(true), path_(std::move(field_path)), op_(op), value_(FieldValue::Array(values)){}
...
bool operator==(const UnaryFilterInternal& lhs, const UnaryFilterInternal& rhs) {
  return lhs.op_ == rhs.op_ && lhs.path_ == rhs.path_ && lhs.value_ == rhs.value_;
}
