...
TEST_F(AggregateQuerySnapshotTest, Equality) {
  CollectionReference collection = Collection({
    {"a", {{"k", FieldValue::String("a")}}},
    {"b", {{"k", FieldValue::String("b")}}},
    {"c", {{"k", FieldValue::String("c")}}}
  });
  AggregateQuerySnapshot snapshot1 = ReadAggregate(collection.WhereEqualTo("k", FieldValue::String("d")).Count());
...
TEST_F(AggregateQuerySnapshotTest, TestHashCode) {
  ...
  EXPECT_EQ(AggregateQuerySnapshotHash(snapshot1), AggregateQuerySnapshotHash(snapshot1));
  EXPECT_NE(AggregateQuerySnapshotHash(snapshot1), AggregateQuerySnapshotHash(snapshot2));
  EXPECT_NE(AggregateQuerySnapshotHash(snapshot1), AggregateQuerySnapshotHash(snapshot3));
  EXPECT_NE(AggregateQuerySnapshotHash(snapshot1), AggregateQuerySnapshotHash(snapshot4));
  EXPECT_NE(AggregateQuerySnapshotHash(snapshot1), AggregateQuerySnapshotHash(snapshot5));
  EXPECT_NE(AggregateQuerySnapshotHash(snapshot1), AggregateQuerySnapshotHash(snapshot6));
  EXPECT_EQ(AggregateQuerySnapshotHash(snapshot2), AggregateQuerySnapshotHash(snapshot2));
  EXPECT_EQ(AggregateQuerySnapshotHash(snapshot2), AggregateQuerySnapshotHash(snapshot3));
  EXPECT_NE(AggregateQuerySnapshotHash(snapshot2), AggregateQuerySnapshotHash(snapshot4));
  EXPECT_NE(AggregateQuerySnapshotHash(snapshot2), AggregateQuerySnapshotHash(snapshot5));
  EXPECT_NE(AggregateQuerySnapshotHash(snapshot2), AggregateQuerySnapshotHash(snapshot6));
  EXPECT_EQ(AggregateQuerySnapshotHash(snapshot4), AggregateQuerySnapshotHash(snapshot4));
  EXPECT_NE(AggregateQuerySnapshotHash(snapshot4), AggregateQuerySnapshotHash(snapshot5));
  EXPECT_NE(AggregateQuerySnapshotHash(snapshot4), AggregateQuerySnapshotHash(snapshot6));
  EXPECT_EQ(AggregateQuerySnapshotHash(snapshot5), AggregateQuerySnapshotHash(snapshot5));
  EXPECT_EQ(AggregateQuerySnapshotHash(snapshot5), AggregateQuerySnapshotHash(snapshot6));
}
