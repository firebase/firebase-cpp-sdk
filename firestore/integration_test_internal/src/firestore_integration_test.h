...
FirestoreInternal* CreateTestFirestoreInternal(App* app, const std::string& database_id = kDefaultDatabase);
...
class FirestoreIntegrationTest : public testing::Test {
 ...
 protected:
  ...
  Firestore* TestFirestoreWithDatabaseId(const std::string& name, const std::string& database_id) const;
 ...
 private:
  ...
  class FirestoreInfo {
   public:
    ...
    FirestoreInfo(const std::string& name, const std::string& database_id, std::unique_ptr<Firestore> firestore)
        : name_(name), database_id_(database_id) ,firestore_(std::move(firestore)) {}
