...
AuthError AuthResponse::error_code() const {
  LogWarning("zzyzx AuthResponse(uid=%d).error_code(); application_data_ is %s", uid(), application_data_ ? "non-null" : "null");
  if (IsSuccessful()) {
...
bool AuthResponse::IsSuccessful() const {
  LogWarning("zzyzx AuthResponse(uid=%d).IsSuccessful(); application_data_ is %s", uid(), application_data_ ? "non-null" : "null");
