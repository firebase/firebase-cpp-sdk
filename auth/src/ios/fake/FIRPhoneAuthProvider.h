...
/** @typedef FIRVerificationResultCallback
    @brief The type of block invoked when a request to send a verification code
   has finished.

    @param verificationID On success, the verification ID provided, nil
   otherwise.
...
/** @class FIRPhoneAuthProvider
    @brief A concrete implementation of `FIRAuthProvider` for phone auth
   providers.
 */
...
/** @fn provider
    @brief Returns an instance of `FIRPhoneAuthProvider` for the default
   `FIRAuth` object.
 */
+ (instancetype)provider NS_SWIFT_NAME(provider());

/** @fn providerWithAuth:
    @brief Returns an instance of `FIRPhoneAuthProvider` for the provided
   `FIRAuth` object.
...
/** @fn verifyPhoneNumber:UIDelegate:completion:
    @brief Starts the phone number authentication flow by sending a verification
   ...
   finished.
    @remarks Possible error codes:
        ...
        + `FIRAuthErrorCodeCaptchaCheckFailed` - Indicates that the reCAPTCHA
...
/** @fn credentialWithVerificationID:verificationCode:
    @brief Creates an `FIRAuthCredential` for the phone number provider
   identified by the verification ID and verification code.
    ...
    @return The corresponding phone auth credential for the verification ID and
   verification code provided.
 */
- (FIRPhoneAuthCredential*)
    credentialWithVerificationID:(NSString*)verificationID
...
#else
/** @fn init
    @brief Please use the `provider` or `providerWithAuth:` methods to obtain an
   instance of `FIRPhoneAuthProvider`.
 */
