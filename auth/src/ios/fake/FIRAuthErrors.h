...
/**
    @brief Errors with one of the following three codes:
        ...
        may contain  an `NSError.userInfo` dictinary object which contains this
   key. The value associated with this key is an NSString of the email address
   of the account that already exists.
 */
extern NSString* const FIRAuthErrorUserInfoEmailKey NS_SWIFT_NAME(AuthErrorUserInfoEmailKey);

/**
    @brief The key used to read the updated Auth credential from the userInfo
...
typedef NS_ENUM(NSInteger, FIRAuthErrorCode) {
  ...
  /** Indicates a network error occurred (such as a timeout, interrupted
     ...
     recoverable with a retry. The `NSUnderlyingError` field in the
     `NSError.userInfo` dictionary will contain the error encountered.
   */
  FIRAuthErrorCodeNetworkError = 17020,

  /** Indicates the saved token has expired, for example, the user may have
     changed account password on another device. The user needs to sign in again
     on the device that made this request.
   */
  ...
  /** Indicates that there are invalid parameters in the payload during a "send
  ...
  // The enum values 17035 is reserved and should NOT be used for new error
  // codes.
  ...
  /** Indicates that the android package name is missing when the
     `androidInstallApp` flag is set to true.
   */
  FIRAuthErrorCodeMissingAndroidPackageName = 17037,

  /** Indicates that the domain specified in the continue URL is not whitelisted
  ...
  // The enum values between 17048 and 17051 are reserved and should NOT be used
  // for new error codes.
  ...
  /** Indicates that the APNs device token could not be obtained. The app may
     not have set up remote notification correctly, or may fail to forward the
     APNs device token to FIRAuth if app delegate swizzling is disabled.
   */
  ...
  /** Indicates that an internal error occurred within a SFSafariViewController
   * or UIWebview.
   */
  ...
  /** Indicates that the local player was not authenticated prior to attempting
     Game Center signin.
   */
  FIRAuthErrorCodeLocalPlayerNotAuthenticated = 17066,

  /** Indicates that a non-null user was expected as an argmument to the
  ...
  /** Indicates that the Firebase Dynamic Link domain used is either not
     configured or is unauthorized for the current project.
   */
  ...
  /** Raised when a JWT fails to parse correctly. May be accompanied by an
     underlying error describing which step of the JWT parsing process failed.
   */
